...
import {
  ...
  adminListCodes, adminUpsertCode, adminDeleteCode,
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      {!token ? (
        ...
        <Dashboard token={token} onLogout={() => { localStorage.removeItem(TOKEN_KEY); setToken(""); }} />
      )}
    </div>
  );
}
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-pink-950 via-slate-950 to-purple-950">
      ...
      >
        <div className="text-center">
          <h1 className="text-2xl font-bold bg-gradient-to-r from-pink-400 to-rose-400 bg-clip-text text-transparent">RosePayFi Admin</h1>
          <p className="text-xs text-slate-400 mt-1">Sign in to manage</p>
        </div>
        <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" placeholder="Email"
          className="w-full bg-slate-800 rounded-lg px-3 py-2 border border-slate-700 focus:border-pink-500 outline-none" required />
        <input value={pass} onChange={(e) => setPass(e.target.value)} type="password" placeholder="Password"
          className="w-full bg-slate-800 rounded-lg px-3 py-2 border border-slate-700 focus:border-pink-500 outline-none" required />
        <button disabled={loading} className="w-full py-2 rounded-lg bg-gradient-to-r from-pink-500 to-rose-500 font-semibold disabled:opacity-50">
          {loading ? "..." : "Sign in"}
        </button>
        <p className="text-[10px] text-slate-500 text-center">Default: admin@rosepayfi.com / RosePayFi2026!</p>
      </form>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen">
      <aside className="md:w-64 bg-slate-900 border-r border-slate-800 p-4">
        <div className="font-bold text-lg mb-4 bg-gradient-to-r from-pink-400 to-rose-400 bg-clip-text text-transparent">🌹 RosePayFi</div>
        <nav className="space-y-1">
          ...
          <button onClick={onLogout} className="w-full text-left px-3 py-2 rounded-lg text-sm text-red-400 hover:bg-slate-800 mt-4">Logout</button>
        </nav>
      </aside>
...
function StatsView({ token }: { token: string }) {
  ...
  useEffect(() => { fn({ data: { token } }).then(setS).catch(() => {}); }, [token]);
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-3">
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search username/telegram_id"
          className="flex-1 bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 outline-none focus:border-pink-500" />
        <button onClick={reload} className="px-4 py-2 bg-pink-500 rounded-lg">Search</button>
      </div>
      <div className="overflow-auto bg-slate-900 rounded-xl border border-slate-800">
        <table className="w-full text-sm">
          <thead className="bg-slate-800 text-xs uppercase text-slate-400">
            <tr>
              <th className="text-left p-2">User</th><th className="text-left p-2">TG ID</th>
              <th className="text-left p-2">Balance</th><th className="text-left p-2">Earned</th>
              <th className="text-left p-2">Refs</th><th className="text-left p-2">Status</th>
          ...
          <tbody>
            {users.map(u => (
              <tr key={u.id} className="border-t border-slate-800">
                ...
                <td className="p-2">{u.suspended ? <span className="text-red-400">Suspended</span> : <span className="text-green-400">Active</span>}</td>
                <td className="p-2 flex gap-1">
                  <button onClick={async () => {
                    await upd({ data: { token, userId: u.id, suspended: !u.suspended, suspendReason: u.suspended ? null as any : "Manual" } });
                    toast.success(u.suspended ? "Unsuspended" : "Suspended"); reload();
                  }} className={`px-2 py-1 rounded text-xs ${u.suspended ? "bg-green-500/20 text-green-300" : "bg-red-500/20 text-red-300"}`}>
                    {u.suspended ? "Unsuspend" : "Suspend"}
                  </button>
                  <button onClick={async () => {
                    const v = prompt("New balance", String(u.balance)); if (v == null) return;
                    await upd({ data: { token, userId: u.id, balance: Number(v) } });
                    toast.success("Balance updated"); reload();
                  }} className="px-2 py-1 rounded text-xs bg-slate-700">Edit Balance</button>
                </td>
              </tr>
            ))}
          </tbody>
...
function TasksView({ token }: { token: string }) {
  ...
  useEffect(() => { reload(); }, [token]);
  const empty = { type: "main", title: "", description: "", amount: 0.1, channel_username: "", channel_url: "", sort_order: 0, active: true };
  return (
    <div>
      <div className="flex justify-between mb-4">
        <h2 className="text-2xl font-bold">Tasks ({tasks.length})</h2>
        <button onClick={() => setEdit(empty)} className="px-4 py-2 bg-pink-500 rounded-lg">+ New Task</button>
      </div>
        ...
        {tasks.map(t => (
          <div key={t.id} className="bg-slate-900 border border-slate-800 rounded-xl p-3 flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-400">{t.amount} ROSE · {t.active ? "active" : "inactive"}</p>
            </div>
            <button onClick={() => setEdit(t)} className="px-3 py-1 text-xs bg-slate-700 rounded">Edit</button>
            <button onClick={async () => { if (!confirm("Delete?")) return; await del({ data: { token, id: t.id } }); toast.success("Deleted"); reload(); }}
              className="px-3 py-1 text-xs bg-red-500/20 text-red-300 rounded">Delete</button>
          </div>
        ))}
      </div>
      {edit && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50" onClick={() => setEdit(null)}>
          <form onClick={e => e.stopPropagation()} onSubmit={async (e) => {
            e.preventDefault();
            await upsert({ data: { token, ...edit, amount: Number(edit.amount), sort_order: Number(edit.sort_order) } });
            ...
            <select value={edit.type} onChange={e => setEdit({ ...edit, type: e.target.value })} className="w-full bg-slate-800 rounded p-2">
              <option value="main">Main</option><option value="partner">Partner</option><option value="other">Other</option>
            </select>
            <input value={edit.title} onChange={e => setEdit({ ...edit, title: e.target.value })} placeholder="Title" className="w-full bg-slate-800 rounded p-2" required />
            <textarea value={edit.description || ""} onChange={e => setEdit({ ...edit, description: e.target.value })} placeholder="Description" className="w-full bg-slate-800 rounded p-2" />
            <input type="number" step="0.01" value={edit.amount} onChange={e => setEdit({ ...edit, amount: e.target.value })} placeholder="Amount ROSE" className="w-full bg-slate-800 rounded p-2" />
            <input value={edit.channel_username || ""} onChange={e => setEdit({ ...edit, channel_username: e.target.value })} placeholder="Channel username (e.g. @channel)" className="w-full bg-slate-800 rounded p-2" />
            <input value={edit.channel_url || ""} onChange={e => setEdit({ ...edit, channel_url: e.target.value })} placeholder="Channel/Open URL" className="w-full bg-slate-800 rounded p-2" />
            <input type="number" value={edit.sort_order} onChange={e => setEdit({ ...edit, sort_order: e.target.value })} placeholder="Sort order" className="w-full bg-slate-800 rounded p-2" />
            ...
            <div className="flex gap-2 pt-2">
              <button type="button" onClick={() => setEdit(null)} className="flex-1 py-2 bg-slate-700 rounded">Cancel</button>
              <button type="submit" className="flex-1 py-2 bg-pink-500 rounded">Save</button>
            </div>
          </form>
        </div>
      )}
  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-2 gap-3">
        {items.map(it => (
          <div key={it.id} className="bg-slate-900 border border-slate-800 rounded-xl p-3 space-y-2">
            <p className="font-semibold">{it.task?.title} <span className="text-xs text-pink-300">+{it.task?.amount} ROSE</span></p>
            <p className="text-xs text-slate-400">@{it.user?.username || it.user?.first_name} · TG: {it.user?.telegram_id}</p>
            {it.screenshot_url && <a href={it.screenshot_url} target="_blank"><img src={it.screenshot_url} className="w-full max-h-48 object-contain rounded border border-slate-700" /></a>}
            <div className="flex gap-2">
              <button onClick={async () => { await review({ data: { token, id: it.id, approve: true } }); toast.success("Approved"); reload(); }}
                className="flex-1 py-1.5 bg-green-500/20 text-green-300 rounded text-sm">Approve</button>
              <button onClick={async () => { const r = prompt("Reject reason?", "Invalid"); if (!r) return; await review({ data: { token, id: it.id, approve: false, reason: r } }); toast.success("Rejected"); reload(); }}
                className="flex-1 py-1.5 bg-red-500/20 text-red-300 rounded text-sm">Reject</button>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div>
      <div className="flex justify-between mb-4">
        ...
        <select value={filter} onChange={e => setFilter(e.target.value)} className="bg-slate-900 rounded px-3 py-1 border border-slate-700">
          ...
          <div key={w.id} className="bg-slate-900 border border-slate-800 rounded-xl p-3">
            <div className="flex justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400">@{w.user?.username || w.user?.first_name} · TG: {w.user?.telegram_id}</p>
                <p className="text-xs font-mono break-all text-slate-300 mt-1">{w.wallet_address}</p>
                <p className="text-xs text-slate-500">Refs: {w.user?.total_ref_count} · Total Withdrawn: {w.user?.total_withdraw}</p>
                {w.tx_id && <p className="text-xs text-green-400 mt-1">TX: {w.tx_id}</p>}
                {w.reject_reason && <p className="text-xs text-red-400 mt-1">Reason: {w.reject_reason}</p>}
              </div>
              <span className={`text-xs px-2 py-1 rounded h-fit ${w.status === "pending" ? "bg-yellow-500/20 text-yellow-300" : w.status === "approved" ? "bg-green-500/20 text-green-300" : "bg-red-500/20 text-red-300"}`}>{w.status}</span>
            </div>
            {w.status === "pending" && (
              <div className="flex gap-2 mt-2">
                <button onClick={async () => {
                  const tx = prompt("Transaction ID / Hash"); if (!tx) return;
                  await review({ data: { token, id: w.id, approve: true, txId: tx } }); toast.success("Approved & posted"); reload();
                }} className="flex-1 py-1.5 bg-green-500/20 text-green-300 rounded text-sm">Approve & Pay</button>
                <button onClick={async () => {
                  const r = prompt("Reject reason?"); if (!r) return;
                  await review({ data: { token, id: w.id, approve: false, reason: r } }); toast.success("Rejected"); reload();
                }} className="flex-1 py-1.5 bg-red-500/20 text-red-300 rounded text-sm">Reject</button>
              </div>
            )}
          </div>
  ...
  return (
    <div>
      <div className="flex justify-between mb-4">
        ...
        <button onClick={() => setEdit(empty)} className="px-4 py-2 bg-pink-500 rounded-lg">+ New Code</button>
      </div>
      <div className="space-y-2">
        {codes.map(c => (
          <div key={c.id} className="bg-slate-900 border border-slate-800 rounded-xl p-3 flex items-center gap-3">
            ...
            <div className="flex-1 text-sm">
              <p>{c.amount} ROSE · {c.used_count}/{c.max_uses} used</p>
              <p className="text-xs text-slate-400">{c.active ? "active" : "inactive"}</p>
            </div>
            <button onClick={() => setEdit(c)} className="px-3 py-1 text-xs bg-slate-700 rounded">Edit</button>
            <button onClick={async () => { if (!confirm("Delete?")) return; await del({ data: { token, id: c.id } }); toast.success("Deleted"); reload(); }}
              className="px-3 py-1 text-xs bg-red-500/20 text-red-300 rounded">Delete</button>
          </div>
        ))}
      </div>
      {edit && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50" onClick={() => setEdit(null)}>
          <form onClick={e => e.stopPropagation()} onSubmit={async (e) => {
            e.preventDefault();
            await upsert({ data: { token, ...edit, amount: Number(edit.amount), max_uses: Number(edit.max_uses) } });
            toast.success("Saved"); setEdit(null); reload();
            ...
            <input value={edit.code} onChange={e => setEdit({ ...edit, code: e.target.value.toUpperCase() })} placeholder="CODE123" className="w-full bg-slate-800 rounded p-2" required />
            <input type="number" step="0.01" value={edit.amount} onChange={e => setEdit({ ...edit, amount: e.target.value })} placeholder="Amount ROSE" className="w-full bg-slate-800 rounded p-2" />
            <input type="number" value={edit.max_uses} onChange={e => setEdit({ ...edit, max_uses: e.target.value })} placeholder="Max uses" className="w-full bg-slate-800 rounded p-2" />
            <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={edit.active} onChange={e => setEdit({ ...edit, active: e.target.checked })} /> Active</label>
            <div className="flex gap-2 pt-2">
              <button type="button" onClick={() => setEdit(null)} className="flex-1 py-2 bg-slate-700 rounded">Cancel</button>
              <button type="submit" className="flex-1 py-2 bg-pink-500 rounded">Save</button>
        ...
        {fields.map(([k, label, def]) => (
          <div key={k as string} className="bg-slate-900 border border-slate-800 rounded-xl p-3">
            ...
            <div className="flex gap-2 mt-1">
              <input value={s[k as string] ?? def} onChange={e => setS({ ...s, [k as string]: e.target.value })}
              ...
              }} className="px-3 bg-pink-500 rounded">Save</button>
            </div>
          </div>
        ))}
      </div>
