    ...
    <div className="px-4 pt-4 pb-28 space-y-4">
      ...
      <div className="glass rounded-2xl p-4 relative overflow-hidden">
        ...
        <p className="text-[10px] text-muted-foreground mt-2">
          🎁 Earn {data.refBonus} ROSE per referral after they complete all main + partner tasks.
          Plus {data.refCommissionPct}% lifetime commission!
        </p>
        <p className="text-[10px] text-amber-300 mt-2">⚠️ {data.antiCheatNote}</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        >
          ...
          <p className="text-xl font-bold mt-1 relative text-emerald-400">
            {data.refs.filter((r: any) => r.status === "claimed").length}
          </p>
        </motion.div>
        <motion.div
          initial={{ y: 10, opacity: 0 }}
        ...
        >
          ...
          <div className="flex items-center gap-1 mt-1 relative">
            ...
            <p className="text-base font-bold">
              {Number(data.user.total_ref_commission).toFixed(2)}
            </p>
          </div>
        </motion.div>
      </div>
        ...
        <AnimatePresence initial={false}>
          {data.refs.map((r: any, i: number) => {
            ...
            const palette =
              status === "claimed"
                ? {
                    ...
                    grad: "from-emerald-500/20 to-transparent",
                    chip: "bg-emerald-500/20 text-emerald-300",
                    label: "✓ Claimed",
                  }
                : status === "claimable"
                  ? {
                      ring: "border-rose-pink/50",
                      grad: "from-rose-pink/30 to-transparent",
                      chip: "bg-rose-pink/20 text-rose-pink",
                      label: "🎁 Claimable",
                    }
                  : {
            ...
            return (
              ...
              >
                <div
                  className={`absolute inset-0 bg-gradient-to-r ${palette.grad} pointer-events-none`}
                />
                {r.referred?.photo_url ? (
                  <img
                    src={r.referred.photo_url}
                    className="w-10 h-10 rounded-full relative ring-2 ring-rose-pink/30"
                  />
                ) : (
                ...
                <div className="flex-1 min-w-0 relative">
                  <p className="text-sm font-medium truncate">
                    @{r.referred?.username || r.referred?.first_name}
                  </p>
                  <span
                    className={`text-[10px] inline-block mt-0.5 px-2 py-0.5 rounded-full ${palette.chip}`}
                  >
                    {palette.label}
                  </span>
                </div>
          ...
          <div className="max-w-md mx-auto pt-8 space-y-2" onClick={(e) => e.stopPropagation()}>
            ...
            {lb.map((u, i) => (
              <div key={i} className="glass rounded-xl p-3 flex items-center gap-3">
                ...
                <span className="text-xs text-rose-pink font-semibold">
                  {u.total_ref_count} refs
                </span>
              </div>
            ))}
          </div>
